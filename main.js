...
class TodoItem {
  constructor(id, text, completed, createAt) {
    this.id = id;
    this.text = text;
...
class TodoApp {
  ...
  savetoStorageAndRender(list) {
    this.todosStorage.setItemToStorage(list);
    this.render();
  }

  addTodo(text) {
    const newTodo = new TodoItem(Date.now(), text, false, new Date().toLocaleDateString());
    this.todoList.push(newTodo);

    this.savetoStorageAndRender(this.todoList);
  }
